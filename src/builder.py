...
def build_VSWL_model(cfg):
    VSWL_model = VSWL.src.models.VSWL_model.VSWL(cfg)
    return VSWL_model

def build_VSWL_from_ckpt(ckpt):
    ...
    VSWL_model = build_VSWL_model(cfg)
...
def build_img_decoder(cfg):
    # decoder_model = models.vision_model.ImageDecoder()
    return VSWL.src.models.vision_model.ImageDecoder(cfg)

def build_text_model(cfg):
    return VSWL.src.models.text_model.BertEncoder(cfg)
...
def build_loss(cfg):
    ...
    if cfg.train.loss_fn.type == "DiceLoss":
        return VSWL.src.loss.segmentation_loss.DiceLoss()
    elif cfg.train.loss_fn.type == "FocalLoss":
        return VSWL.src.loss.segmentation_loss.FocalLoss()
    elif cfg.train.loss_fn.type == "MixedLoss":
        return VSWL.src.loss.segmentation_loss.MixedLoss(alpha=cfg.train.loss_fn.alpha)
