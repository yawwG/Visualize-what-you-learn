...
def np_to_pil2(input, img_np):
    ...
    ar = np.uint8(img * 255)
    ar = transform.resize(ar, (512, 512))
    ar = skimage.img_as_float(ar)  # 先转换成uint16的格式
...
def build_attention_images(
    ...
    if rand_vis:
        ...
        # loop_idx = np.random.choice(len(attn_maps), size=nvis, replace=False)
    else:
        # loop_idx = np.arange(nvis)
        loop_idx = np.arange(len(real_imgs))
